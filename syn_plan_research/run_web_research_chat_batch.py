...
async def main_async(args):
    ...
    result_json_name = f'{args.split}.{t.tm_mon}.{t.tm_mday},{t.tm_hour}:{t.tm_min}.{random_num}'
    ...
    subset_idx = 0
    assert len(questions) == len(data), "Questions and data length mismatch"
    for batch_idx in range(len(questions)):
        batch_questions = questions[batch_idx:batch_idx + 16*100]
        batch_data = data[batch_idx:batch_idx + 16*100]

        tasks = [agent.run(q) for q in batch_questions] 
        ...
        out_seqs = []
        assert len(completed_sequences) == len(batch_data)
        for seq, item in zip(completed_sequences, batch_data):
        ...
        if args.pass_at_k > 1:
            cur_result_json_name = f'pass_at_k_{args.pass_at_k}-{result_json_name}'

        if args.prompt_type != "original":
            cur_result_json_name = f'prompt_{args.prompt_type}-{result_json_name}'

        if args.use_bedrock:
            if "claude-3-7" in args.model_name.lower():
                cur_result_json_name = f'claude3-7-{cur_result_json_name}'
            else:
                raise ValueError(f"Unknown model name for Bedrock: {args.model_name}")
            
        cur_result_json_name = f'{cur_result_json_name}.{subset_idx}.json'
        subset_idx += 1

        with open(os.path.join(output_dir, cur_result_json_name), 'w', encoding='utf-8') as f:
