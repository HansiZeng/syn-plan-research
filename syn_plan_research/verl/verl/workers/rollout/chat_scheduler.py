...
class ChatCompletionScheduler:
    ...
    def submit_chat_completions(self, *, messages: List[Dict[str, str]], request_id: str, info: Dict[str, Any], filter_convs: List[float] = None):
        ...
        """
        info["__depth__"] += 1
        task = asyncio.create_task(self._submit_chat_completions_and_callback(messages, request_id, info, filter_convs))
    ...
    async def _submit_chat_completions_and_callback(
        ...
        try:
            # print(f"🐶 [DEBUG] ChatCompletionScheduler sampling_params {info['__sampling_params__']}")
        ...
        if exception is not None:
            ...
            try:
                await self.completion_callback(messages, completions, info, filter_convs=filter_convs)
            except Exception as e:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        # New add from 08/04/2025 
        filter_convs = [[1.0] for _ in range(len(batch) * n)]
            ...
            tasks.append(
                asyncio.create_task(
                    self._submit_chat_completions_semaphore(
                        ...
                        filter_convs=filter_convs[batch_index]
                    )
                )
            )
        
        if not self.config.filter_unfinished:
            # We set back to 1.0 for all conversations 
            filter_convs = [[1.0]] * len(batch_conversations)

        await asyncio.gather(*tasks)
        filter_convs = [item[0] for item in filter_convs]  # Extract the first element from each list
        output_batch = self.completion_callback.postprocess(batch, batch_conversations, n=n, response_filter=filter_convs)
    ...
    async def _submit_chat_completions_semaphore(
        self, messages: List[Dict[str, str]], request_id: str, sampling_params: Dict[str, Any], filter_convs: List[float] = None
    ):
        done = asyncio.Event()
        ...
        self.submit_chat_completions(messages=messages, request_id=request_id, info=info, filter_convs=filter_convs)
