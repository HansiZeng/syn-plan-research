...
# from MFiveLib.mfive.apps.iefm.arm import trainer
import hydra
import ray
...
from verl.trainer.ppo.my_ray_trainer import RayPPOTrainer
...
def save_resolved_config(config, save_dir):
    """Save resolved OmegaConf config to a YAML file in the given save_dir."""
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, "resolved_config.yaml")
    with open(save_path, "w") as f:
        OmegaConf.save(config=config, f=f)
    print(f"✅ Saved resolved config to {save_path}")
...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ...
        if config.trainer.is_multi_node:
            ray.init(
                address="auto",                      # ⬅ 这句非常重要！连到已有 Ray 集群
                num_cpus=config.ray_init.num_cpus,
            )
            print("using multinode to initalize ")
        else:
            ray.init(
...
class TaskRunner:
    def run(self, config):
        ...
        from verl.trainer.ppo.my_ray_trainer import ResourcePoolManager, Role
        ...
        # )
        val_reward_fn = EvalNaiveRewardManager(
            tokenizer=tokenizer,
            num_examine=1,
            compute_score=None,
        ...
        if config.tool_server.crawl_webpage_server.server_num == 1:
            ...
            from tests.workers.rollout.my_tools_sever import CrawlWebpageToolServerV2 
            crawl_actors = [
                CrawlWebpageToolServerV2.options(name=f"crawl_v2_{i}", num_cpus=1).remote(
                    semaphore_limit=config.tool_server.crawl_webpage_server.per_server_semaphore_limit,
                    cache_file=config.tool_server.crawl_webpage_server.cache_file,
                    snippet_cache_file=None
                )
                for i in range(config.tool_server.crawl_webpage_server.server_num)]
        ...
        if config.tool_server.crawl_webpage_server.server_num == 1:
            ...
            crawl_addr = [ray.get(crawl_actor.get_server_address.remote()) for crawl_actor in crawl_actors]
            print(f"✅ CrawlWebpageToolServer started at {crawl_addr}")
            config.tool_server.crawl_webpage_server.url = [f"http://{addr}" for addr in crawl_addr]
            config.tool_server.crawl_webpage_server.parameters = ray.get(crawl_actors[0].get_parameters.remote())
            crawler_metadata = ray.get(crawl_actors[0].get_metadata.remote())
        ...
        train_dataset = create_rl_dataset(config.data.train_files, config.data, tokenizer, processor, tool_metadata_map)
        val_dataset = create_rl_dataset(config.data.val_files, config.data, tokenizer, processor, tool_metadata_map)
        train_sampler = create_rl_sampler(config.data, train_dataset)

        save_resolved_config(config, save_dir=config.trainer.default_local_dir)
        ...
        trainer = RayPPOTrainer(
            ...
            cache_saver_actor=cache_saver_actor,
        )
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor,
                      tool_metadata_map=None, prompt_fn=None,
                      ):
    """Create a dataset.
    ...
    if "custom_cls" in data_config and data_config.custom_cls.get("path", None) is not None:
        ...
        raise ValueError("Custom dataset class is not specified in the data configuration.")
