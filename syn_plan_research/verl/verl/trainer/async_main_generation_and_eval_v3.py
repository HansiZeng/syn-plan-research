...
def main_task(config):
    ...
    try:
        ...
        if config.tool_server.crawl_webpage_server.server_num == 1:
            crawl_actor = CrawlWebpageToolServer.options(
                name=f"crawl_webpage_server_{uniq}",
                max_restarts=0,
                max_task_retries=0,
            ).remote(
            ...
            crawl_actors = [
                CrawlWebpageToolServerV2.options(
                    name=f"crawl_v2_{i}_{uniq}",
                    num_cpus=1,
                    max_restarts=0,
                    max_task_retries=0,
                ).remote(
        ...
        if config.tool_server.crawl_webpage_server.server_num == 1:
            ...
            crawl_addr = [ray.get(a.get_server_address.remote()) for a in crawl_actors]
            print(f"✅ CrawlWebpageToolServer started at {crawl_addr}")
        ...
        _kill_actor(crawl_actor)
        _kill_actor(crawl_actors)
