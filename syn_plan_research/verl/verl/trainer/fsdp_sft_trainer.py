...
from verl.utils.dataset.qwen3_multiturn_sft_dataset import Qwen3MultiTurnSFTDataset as MultiTurnSFTDataset
...
def run_sft(config):
    ...
    debug_path = os.path.join(os.path.dirname(config.data.train_files), "sft_debug_output.json")
    print(debug_path)
    debug_data = []

    for i in range(min(50, len(train_dataset))):  # 前 50 条样本
        sample = train_dataset[i]
        debug_data.append({
            "index": i,
            "input_ids": sample["input_ids"].tolist(),
            "attention_mask": sample["attention_mask"].tolist(),
            "position_ids": sample["position_ids"].tolist(),
            "loss_mask": sample["loss_mask"].tolist(),
            # Optional: decode to text
