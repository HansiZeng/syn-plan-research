...
# def compute_passk_stats(df: pd.DataFrame, score_col: str = "reward"):
#     """
#     给定一个包含 (id, data_source, score_col) 的 DataFrame，
#     计算 pass@k 相关的 max/mean 统计。

#     Args:
#         df: 原始 DataFrame，至少包含 ['id', 'data_source', score_col]
#         score_col: 分数列名（默认 'reward'）

#     Returns:
#         per_pair_df: 每个 (id, data_source) 的指标
#         per_source_df: 每个 data_source 聚合后的指标
#     """
#     # 确保分数列是数值型
#     df[score_col] = pd.to_numeric(df[score_col], errors="coerce")

#     # 1) 按 (id, data_source) 聚合
#     per_pair_df = (
#         df.groupby(["id", "data_source"], as_index=False)
#           .agg(
#               pass_k=(score_col, "size"),
#               score_max=(score_col, "max"),
#               score_mean=(score_col, "mean"),
#           )
#     )

#     # 2) 按 data_source 聚合（对不同 id 的指标取平均）
#     per_source_df = (
...
def prepare_data_files(val_files_input):
    """
    自动判断 val_files_input 是本地路径还是 HuggingFace dataset name
    
    Args:
        val_files_input: 可以是:
            1. 字符串列表的字符串表示: "['path1.parquet', 'path2.parquet']"
            2. HuggingFace dataset name: "hzeng/syn-plan-research-data-eval"
            
    Returns:
        list: 本地 parquet 文件路径列表
    """
    import ast
    
    # 尝试解析为列表
    try:
        if val_files_input.startswith('[') and val_files_input.endswith(']'):
            # 这是一个列表字符串
            data_files = ast.literal_eval(val_files_input)
            print(f"✅ Parsed as file list: {data_files}")
            
            # 检查是否都是本地文件
            all_local = all(Path(f).exists() or f.startswith('/') for f in data_files)
            if all_local:
                print(f"✅ Using local parquet files: {data_files}")
                return data_files
            else:
                print(f"⚠️ Some files in list don't exist locally: {data_files}")
                return data_files
        else:
            # 可能是 HuggingFace dataset name
            print(f"🔍 Detected potential HuggingFace dataset: {val_files_input}")
            return _download_hf_dataset(val_files_input)
            
    except (ValueError, SyntaxError) as e:
        # 不是列表格式，尝试作为 HuggingFace dataset
        print(f"🔍 Not a list format, trying as HuggingFace dataset: {val_files_input}")
        return _download_hf_dataset(val_files_input)


def _download_hf_dataset(dataset_name):
    """
    从 HuggingFace 下载 dataset 并保存为临时 parquet 文件
    
    Args:
        dataset_name: HuggingFace dataset 名称，如 "hzeng/syn-plan-research-data-eval"
        
    Returns:
        list: 包含临时 parquet 文件路径的列表
    """
    try:
        ...
        # 创建临时文件
        temp_dir = tempfile.mkdtemp(prefix="hf_dataset_")
        temp_file = Path(temp_dir) / "data.parquet"
        
        # 保存为 parquet
        dataset.to_parquet(str(temp_file))
        print(f"✅ Saved HuggingFace dataset to temporary file: {temp_file}")
        
        return [str(temp_file)]
        
    except Exception as e:
        print(f"❌ Failed to load HuggingFace dataset '{dataset_name}': {e}")
        raise ValueError(f"Could not load data from '{dataset_name}'. "
                        f"Please check if it's a valid HuggingFace dataset name or local path.")
...
def run_generation(config) -> None:
    if not ray.is_initialized():
        ...
        ray.init(
            runtime_env={"env_vars": {"TOKENIZERS_PARALLELISM": "true", "NCCL_DEBUG": "WARN", "VLLM_LOGGING_LEVEL": "WARNING",
                "VLLM_USE_V1": "1"}},
...
def main_task(config):
    ...
    # create rl dataset:
    # 自动判断并准备数据文件
    print(f"🔍 Processing data input: {config.data.val_files}")
    data_files = prepare_data_files(config.data.val_files)
    print(f"✅ Final data files to use: {data_files}")
    
    processor = None
    val_dataset = create_rl_dataset(data_files, config.data, tokenizer, processor, tool_metadata_map) 
    if config.validation.pass_at_k > 1:
        ...
        if config.validation.data_source:
            print(f"🔍 Filtering dataset by source '{config.validation.data_source}' for pass@k evaluation.")
